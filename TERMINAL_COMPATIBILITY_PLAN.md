...
| 133 `k=s` / `k=c` / `k=i` / `k=r` | The prompt's kind | ✅ | the field naming what kind of prompt a mark opens, on whichever letter carries it. The proposal defines all four on `P` — *"regular primary prompt (`k=i` or default), right-side prompts (`k=r`), or prompts for continuation lines (`k=c` or `k=s`)"* — and kitty's shell integration puts `k=s` on `A`, prepending it to zsh's `PS2` once per line of a command still being typed. **Only a continuation is not a prompt start**, and both its spellings suppress the mark: treating one as a new prompt litters the gutter with ticks and files the finished command against its last continuation line instead of its prompt. `k=i` is the primary prompt and keeps its mark; `k=r` keeps its too, a right-side prompt sharing the line of the prompt it decorates, so its anchor is one `record` already holds. `k=c` was read as unknown until §164 — that is, as a prompt — which is the recoverable half of §97's rule working as designed, and it is now read as what it is. An unknown value still stays a prompt start, losing an anchor being worse than gaining one (§97, §164, `term/osc133.rs`) |
