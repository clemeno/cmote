...
| 133 (any other phase letter) | Further prompt phases | ❌ | the letter space past `A`/`B`/`C`/`D`/`N`. This row read *"the reachable accounts disagree about what they mean"* until §164, which found the accounts were write-ups and went to the thing they are write-ups **of** — Per Bothner's semantic-prompts proposal, which defines `P`, `I` and `L` too and gives each one meaning. `P` and `L` are still unanswered here and take their own rows as they are settled. A letter the proposal does not define yields no mark rather than the nearest guess, because a wrong mark moves a prompt jump or mis-bounds a command's output where no mark leaves both alone (§96, §97, §164, `term/osc133.rs`) |
