...
| 133 `A ; cl=m` | Multi-line prompt hint | ❌ | VS Code's field on the prompt-start mark, saying the prompt runs over several lines. Ignored at no cost: a prompt jump anchors on the `A` mark's own line, which is the prompt's first line whether the hint is there or not (§96, `term/osc133.rs`) |
| 133 (any other phase letter) | Further prompt phases | ❌ | the letter space past `A`/`B`/`C`/`D` — Konsole is documented as tracking the prompt as "A/N/P", and no reachable source gives `N` or `P` a syntax or a meaning. An unrecognised letter yields no mark rather than the nearest guess, because a wrong mark moves a prompt jump or mis-bounds a command's output where no mark leaves both alone (§96, `term/osc133.rs`) |
