...
| 133 | Shell integration (semantic prompts) | ✅ | `OSC 133 ; A/B/C/D`, BEL- or ST-terminated, marks where the prompt, the command and its output begin and end — and since §164 `N`, `P` and `I` as well, the proposal's other three prompt letters, each on its own row below; drives the per-tab status dot, jump-to-prompt and select-command-output, exit code from the optional field after `D` — optional in the grammar, `"D", [ ";", exitcode ]`, with the bare form a documented spelling rather than a tolerated malformation. Trailing `key=value` fields are ignored except the prompt's kind; the four named ones have their own rows below (§34, §95, §96, §164, `term/osc133.rs`) |
| 133 `N` | Prompt start, terminating the last command | ✅ | the proposal's *"Same as `OSC "133;A"` but may first implicitly terminate a previous command"* — Konsole's letter, and one of the four this table called unreadable until §164. Read as a prompt start, sharing `A`'s arm rather than growing a variant of its own: the termination is keyed on an `aid`, and *"if no `aid` is specified, treat as an `aid` whose value is the empty string"* — cmote holds no `aid`, so every command it has carries the empty one and the termination is unconditional. That is what a prompt start already does here, superseding a half-built command instead of filing it, which is why `N` is `A` and the ARM says so. The `k=` fields are read the same way, a letter defined as "same as `A`" having no licence to read its own differently (§34, §97, §164, `term/osc133.rs`) |
| 133 `P` | Explicit prompt start | ✅ | the proposal's *"Explicit start of prompt. Optional after an `A` or `N` command"* — the letter that starts a prompt without starting a command, and the one the `k` field is actually defined on. Read as a prompt start since §164, sharing `A`'s arm. **This letter is why the row below used to say the accounts disagree**, and reading the proposal dissolves all three at once: a zsh write-up's `133;P;k=i` for `PS1` and `133;P;k=s` for `PS2` is the kind taxonomy applied, Konsole's "A/N/P" is three spellings of one event, and a Ghostty fork's bare `133;P` for a prompt *redraw* costs nothing here — a redraw lands on the line the prompt is already anchored at, and `record` drops a repeat of the last line. **One divergence, named by §165 and not built for**: the proposal makes `P` *"optional after an `A` or `N`"*, so a shell may send both, and both are prompt starts here — on different rows the first would leave a spare tick resolving to no command, since `record` drops only a repeat of the *last* line. They land on different rows only if something moved the cursor between them, and the thing that would is the fresh-line `A` specifies and cmote does not perform (see the `L` row); a shell writing its own `\r\n` between the two is the case that costs a tick. Recognising the second start as redundant means holding a prompt phase across marks, which is machinery for a stream nobody has produced (§34, §97, §164, §165, `term/osc133.rs`) |
| 133 `I` | Prompt end, input to end of line | ✅ | the proposal's *"End of prompt and start of user input, terminated by end-of-line"* — `B`'s sibling, differing only in where the user's INPUT region ends: at the next `C` or prompt for `B`, at the line's end for `I`. Read as a prompt end since §164, sharing `B`'s arm, and **the difference is not modelled**: `PromptEnd` moves nothing here, the phase staying the prompt's until output begins, so cmote holds no input region for the two to end differently in. Named rather than hidden — it would start to matter the day cmote showed the command line, which it refuses to on the second-source rule (§71, §97, §164, `term/osc133.rs`) |
| 133 `L` | Fresh line | 🛑 | the proposal's *"Do a fresh-line: If the cursor is the initial column (left, assuming left-to-right writing), do nothing. Otherwise, it does the equivalent of `"\r\n"`"* — **the one letter of the family that is not a mark**. Every other tells cmote something *about* text the engine is already drawing; this one tells cmote to draw, on a channel whose declared job is saying where the prompt sits, which is the ground `click_events=1` is refused on one row down — there a field, here the letter. It would break §34's own arrangement: the scanner observes and is a pure bytes → marks function with no engine in it, `term::mod` places what it finds once the engine has reached each offset, the engine draws. Honouring it makes the observer a writer — a `Mark` variant naming an action rather than a phase, `apply` grown a cursor COLUMN it does not take, and a second thing putting bytes on the grid (§71, §73). **The cost is real and stated**: a stream that leans on it gets its prompt on the current line where another terminal would break the line first — and cmote does not do the fresh-line `A` also specifies (*"First do a fresh-line. Then start a new command"*), so this is one decision applied twice rather than a special case. The refusal is the letter list, an allow-list that drops what it does not name; there is no arm for `L`, an arm returning nothing beside a wildcard returning nothing being a branch that never branches — clippy's `match_same_arms` refused the first draft of this row's code and was right. What names the letter is a **test**, which fails the day it is wired to a mark (§34, §71, §95, §164, `term/osc133.rs`) |
...
| 133 `k=s` / `k=c` / `k=i` / `k=r` | The prompt's kind | ✅ | the field naming what kind of prompt a mark opens, on whichever letter carries it. The proposal defines all four on `P` — *"regular primary prompt (`k=i` or default), right-side prompts (`k=r`), or prompts for continuation lines (`k=c` or `k=s`)"* — and kitty's shell integration puts `k=s` on `A`, prepending it to zsh's `PS2` once per line of a command still being typed. **Only the primary kind opens a prompt** — `k=i`, no `k=` at all, or a value from no source. The other three suppress the mark, because treating one as a new prompt litters the gutter with ticks and files the finished command against its last continuation line instead of its prompt. `k=c` joins `k=s`, being the proposal's own second spelling of the same kind; it was read as an *unknown* value until §164, which is the recoverable half of §97's rule working as designed. **`k=r` is suppressed too, and §164 first got that wrong**: it kept the mark on the reasoning that a right prompt is drawn on the line the prompt it decorates already anchored, which is true only when that prompt was itself *marked* — and on a continuation line it was not, so a `k=r` there would tick the gutter and re-seat the pending span, which is §97's harm let back in through the field beside the one that keeps it out. §97's asymmetry does not rescue it: that rule is about an *unknown* kind, where a wrong guess costs a spare tick, while a known `k=r` costs the pending span as well. An unknown value does still stay a prompt start (§97, §164, `term/osc133.rs`) |
...
| 133 `A ; cl=` / `move-keys=` | Click-to-move in the input area | 🛑 | **This row said the wrong thing until §164.** It read "`cl=m` — multi-line prompt hint … saying the prompt runs over several lines", refused it as restating a fact cmote can already see, and concluded *"nothing would change if it were honoured"*. The proposal says otherwise: `cl=` is *"a request from the application to the terminal to translate clicks in the input area to cursor movement"*, its four values differing in how far that motion may reach — `line` within one input line, `m` between lines using only left/right, `v` the same with up/down, `w` the same again with the application handling smart vertical movement. The `m` is *movement*, not *multi-line*, and honouring it means cmote **synthesising arrow-key sequences to the pty when the user clicks**. Its companion `move-keys=` *left* `,` *right* [`,` *up* `,` *down*] then lets the sender **choose which bytes** those are, the defaults being *"the standard arrow-key sequences: `CSI D` etc."* So the correct refusal is not the second-source rule at all but the row above's: input manufactured on a channel whose declared job is saying where the prompt sits, around the modes that gate input reporting (§10, §95) — and worse than `click_events=1`, which reports a click where this one fabricates keystrokes for it. Ghostty pairs the two as one feature, shipping *"the `click-events` and `cl=line` extensions"* in 1.3.0. cmote refuses both, structurally: nothing here turns a field into input. **Not "every trailing field is dropped"**, which is what this row said until §165 and is contradicted two rows up — `k=` is read, and `Mark::CommandEnd` carries `D`'s exit code. The accurate statement is narrower and stronger: the only field read decides whether a mark is *made*, the only field carried is a number shown on a tab, and neither reaches the pty (§10, §95, §97, §164, §165, `term/osc133.rs`) |
| 133 (any other phase letter) | Further prompt phases | 🛑 | the alphabet past the eight the proposal defines. **Read ❌ until §164**, under a note saying *"a letter cannot be supported or refused until it means one thing"* and citing three reachable accounts that *"disagree about what they mean"* — the accounts being write-ups, and the thing they are write-ups **of** being Per Bothner's semantic-prompts proposal, which §164 read. It defines `A`, `B`, `C`, `D`, `I`, `L`, `N` and `P`, each with one meaning, and the three reports turn out to agree; the rows above answer all eight. What is left is a letter **no source defines**, which cannot be a gap because there is nothing to build — the mark it deserves is the one for what cmote does when one arrives, and what cmote does is decide. `parse`'s letter list is an allow-list, the same construction `term/dsr.rs`, `term/iterm.rs`, `term/pointer.rs` and `term/modkeys.rs` are refused through, and a test now walks the whole alphabet minus the eight to say how wide it is. An undefined letter yields no mark rather than the nearest guess, because a wrong mark moves a prompt jump or mis-bounds a command's output where no mark leaves both alone (§96, §97, §164, `term/osc133.rs`) |
...
| ReGIS | ❌ | DEC's vector graphics language, drawn by an interpreted command stream in a `DCS … p` envelope — framed by `term/dcs.rs` like every other DCS and claimed by no reader, both graphics scanners requiring final byte `q` (`graphics::is_sixel`). Still a gap, and §163 re-priced it rather than repeating part 5: the cost is an interpreter for curves, fills, screen addressing and text, for a language essentially nothing emits — YAGNI with a real price rather than an afternoon nobody has spent. **What the re-pricing found** is that a conforming implementation could not be whole anyway: ReGIS' `@` command stores up to **10,000 characters** of command stream in 26 letter-named **macrographs**, and its Report command's `M` option reads one back. A remote-named store of stored programs, with readback, is the class part 6 refuses outright — DECDMAC / DECDLD / DECUDK, GIP's `o=u` pool (§155), and the macro-space reports §82 declines. So ReGIS would ship with a hole cmote chooses, which is worth knowing before the interpreter is costed again (part 5, §12, §82, §98, §155, §163) |
...
be three partial views of `P`. So the row became five: `N`, `P` and `I` are ✅, each mapping onto a
...
`iterm.rs`, `pointer.rs` and `modkeys.rs` rows are all 🛑 for. A **separate, pre-existing** row was
corrected rather than answered — not one this row became, which is a distinction §165 had to make
after §164 counted it as a sixth: `cl=` is not a hint about the prompt's shape, it asks the terminal
to manufacture arrow keys on a click, and its companion `move-keys=` lets the sender choose them.
...
- **ReGIS' macrographs, read for the first time in §163** — the VT330/VT340 Programmer Reference
  Manual, chapter 9 (`vt100.net/docs/vt3xx-gp/chapter9.html`). ReGIS' `@` command "defines a command
  string as a macrograph … used to store and recall other ReGIS command strings": `@:<call letter>` and
  `@;` bracket a definition, `@<call letter>` invokes it, `@.` clears all twenty-six, and **"up to
  10,000 characters of code can be stored"**. Chapter 10's Report command carries an `M` option, which
  "tells ReGIS to report the contents of a specified macrograph", replying `@=<call letter>…`. That
  pair — a remote-named store of stored programs, and a readback of it — is what the ReGIS row now
  says would be refused even if the interpreter were built, and it is the same class as DECDMAC /
  DECDLD / DECUDK and GIP's `o=u`.
